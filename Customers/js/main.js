...
$(document).ready(function () { // wait for document to be ready
  $.getJSON('js/grid_Data.js').done(function(data) {
    ...
    var customerList = data.fs_DATABROWSE_F0101.data.gridData.rowset.map(function(row) {
      console.log(row['']);
      return{
        type : row['F0101_AT1'],
        customerName : row['F0101_ALPH']
      }

    });
  resultsElem.textContent = JSON.stringify(customerList, null, 3);
  });
});
//
//     var req = {}; // empty object to hold our http request
//     req.deviceName = 'aisTester654321'; // <<---- here change to a unique name
//     req.username = "demo";
//     req.password = "demo";
//
